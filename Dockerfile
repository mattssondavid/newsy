FROM wordpress:5.0.2-php7.2-apache

# https://this.localhost SSL
# Point your network hosts to: 127.0.0.1  this.localhost
COPY etc/ssl/create_domain_cert.sh /root/
COPY etc/ssl/rootCA.pem /root/
COPY etc/ssl/rootCA.key /root/
COPY etc/ssl/v3.ext /root/
